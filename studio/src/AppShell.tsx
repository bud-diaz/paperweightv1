...
import { TipConfigModal } from '@/components/TipConfigModal';
...
import { toDisplayTracks } from '@/lib/library';
import type { CreatorPost, ModalKey, ModeKey, Track, ViewKey } from '@/types';

const modeOrder: ModeKey[] = ['stack', 'play', 'studio'];

function modeDirection(current: ModeKey, next: ModeKey) {
  return Math.sign(modeOrder.indexOf(next) - modeOrder.indexOf(current));
}
...
export function AppShell() {
  ...
  const [uploadArtFile, setUploadArtFile] = useState<File | null>(null);
  ...
  const { data: libraryStructure } = useQuery<LibraryStructure>({ queryKey: ['library', 'structure'], queryFn: () => api.library.structure() });
  const filteredTracks = useMemo(() => toDisplayTracks(libraryStructure).filter((track) => `${track.title} ${track.collection}`.toLowerCase().includes(search.toLowerCase())), [libraryStructure, search]);
  ...
  const switchMode = (nextMode: ModeKey) => {
    if (nextMode !== mode) setModeTransitionDirection(modeDirection(mode, nextMode));
    setMode(nextMode);
    setMobileMenu(false);
  };
  const modeViewVariants = {
  ...
  // Cross-links Releases <-> Vault: jump to the other view already focused
  // on the same collection, rather than landing on a generic tab the
  // creator has to re-locate the collection in themselves.
  const goToProject = (targetView: 'releases' | 'vault', projectId: number) => { setFocusProjectId(projectId); navigate(targetView); };
  ...
  const updatePost = useMutation({
    mutationFn: () => api.dashboard.posts.update(editingPost!.id, { body: postBody.trim(), visibility: postVisibility }),
    onSuccess: ({ res, data: result }: { res: Response; data: { error?: string } }) => {
      if (!res.ok) { notify(result.error || 'Failed to update post.'); return; }
      queryClient.invalidateQueries({ queryKey: ['dashboard', 'posts'] });
      setEditingPost(null);
      setPostBody('');
      notify('Post updated.');
    ...
    onError: () => notify('Failed to update post.'),
  });
  const deletePost = useMutation({
    mutationFn: (post: CreatorPost) => api.dashboard.posts.remove(post.id),
    onSuccess: ({ res, data: result }: { res: Response; data: { error?: string } }) => {
      if (!res.ok) { notify(result.error || 'Failed to delete post.'); return; }
      queryClient.invalidateQueries({ queryKey: ['dashboard', 'posts'] });
      notify('Post deleted.');
    },
    onError: () => notify('Failed to delete post.'),
  });
  const openPostEditor = (post: CreatorPost) => { setEditingPost(post); setPostBody(post.body); setPostVisibility(post.visibility); setModal('posts'); };
  const handleDeletePost = (post: CreatorPost) => {
    if (window.confirm('Delete this post? It will disappear from your public feed immediately.')) deletePost.mutate(post);
  };
  const uploadMutation = useMutation({
    ...
    onSuccess: async ({ res, data }: { res: Response; data: { error?: string; id?: number | null; filename?: string; title?: string | null } }) => {
      if (!res.ok) { notify(data.error || 'Upload failed.'); return; }
      if (data.id && uploadCollectionId) {
        const { res: collectionRes } = await api.dashboard.vault.addTrack(uploadCollectionId, { content_id: data.id });
        if (!collectionRes.ok) notify('Uploaded, but couldn’t add it to that collection.');
      }
      if (data.id && uploadArtFile) {
        const artFormData = new FormData();
        artFormData.append('artwork', uploadArtFile);
        const { res: artRes } = await api.dashboard.media.uploadArtwork(data.id, artFormData);
        if (!artRes.ok) notify('Uploaded, but couldn’t attach the cover art.');
      }
      queryClient.invalidateQueries({ queryKey: ['library', 'structure'] });
      notify(`${data.title || data.filename} added to your library.`);
      setUploadFile(null);
      setUploadTitle('');
      setUploadArtFile(null);
  ...
  const modalContent = () => {
    ...
    if (modal === 'newShareLink') {
      const shareProjects = libraryStructure?.projects || [];
      const shareTracks = libraryStructure?.standalone || [];
    ...
    if (modal === 'share') return <Modal title="Send them somewhere good." eyebrow="Share" onClose={closeModal}><div className="panel-subtle rounded-xl p-5"><div className="flex items-center gap-3"><div className="h-12 w-12 rounded-xl flex items-center justify-center bg-primary"><Music2 size={19} className="text-primary-foreground" /></div><div><p className="font-medium">{stationName} — Radio</p><p className="text-xs text-muted-foreground mt-1">Your station's public listening link</p></div></div><div className="mt-5 flex gap-2 rounded-xl border border-white/[.08] bg-black/20 p-2"><input readOnly value={window.location.href} data-testid="input-share-link" onFocus={(event) => event.currentTarget.select()} className="min-w-0 flex-1 bg-transparent px-2 text-xs text-muted-foreground outline-none" /><button type="button" data-testid="button-copy-share-link-inline" onClick={async () => { try { if (!navigator.clipboard) throw new Error('Clipboard API unavailable'); await navigator.clipboard.writeText(window.location.href); notify('Share link copied.'); } catch { notify('Copy failed — select the link and copy it manually.'); } }} className="lime-button rounded-lg px-3 py-2 text-xs font-semibold flex items-center gap-2"><Copy size={14} /> Copy</button></div></div><div className="grid grid-cols-2 gap-3 mt-5"><button type="button" data-testid="button-copy-share-link" onClick={async () => { try { if (!navigator.clipboard) throw new Error('Clipboard API unavailable'); await navigator.clipboard.writeText(window.location.href); notify('Share link copied.'); closeModal(); } catch { notify('Copy failed — select the link and copy it manually.'); } }} className="lime-button rounded-xl py-3 text-sm font-semibold flex items-center justify-center gap-2"><Copy size={15} /> Copy link</button><button type="button" data-testid="button-share-social" onClick={async () => { if (navigator.share) { try { await navigator.share({ title: stationName, text: `${stationName} — listen on Paperweight`, url: window.location.href }); closeModal(); return; } catch (error) { if ((error as DOMException)?.name === 'AbortError') return; } } try { if (!navigator.clipboard) throw new Error('Clipboard API unavailable'); await navigator.clipboard.writeText(window.location.href); notify('Share link copied.'); closeModal(); } catch { notify('Copy failed — select the link and copy it manually.'); } }} className="ghost-button rounded-xl py-3 text-sm flex items-center justify-center gap-2"><Share2 size={15} /> More options</button></div><div className="mt-6 flex items-center justify-between text-xs text-muted-foreground"><span>Public sharing</span><span className="text-primary flex items-center gap-1"><Globe2 size={13} /> On</span></div></Modal>;
    ...
    if (modal === 'posts') { const isEditing = !!editingPost; return <Modal title={isEditing ? 'Touch it up.' : 'Say something true.'} eyebrow="Activity / Post" onClose={() => { setEditingPost(null); setPostBody(''); closeModal(); }}><Field label="Post" value={postBody} onChange={setPostBody} placeholder="A note for the people listening..." multiline /><label className="flex items-center gap-2 mt-5 text-xs text-muted-foreground">Visible to<select data-testid="select-post-visibility" value={postVisibility} onChange={(event) => setPostVisibility(event.target.value as 'public' | 'supporters_only')} className="input-studio rounded-lg px-2 py-1.5 ml-2"><option value="public">Everyone</option><option value="supporters_only">Supporters only</option></select></label><div className="flex justify-end mt-6"><button type="button" data-testid="button-publish-post" onClick={() => (isEditing ? updatePost.mutate() : publishPost.mutate())} disabled={(isEditing ? updatePost.isPending : publishPost.isPending) || !postBody.trim()} className="lime-button rounded-lg px-4 py-2 text-xs font-semibold disabled:opacity-50">{isEditing ? (updatePost.isPending ? 'Saving…' : 'Save changes') : (publishPost.isPending ? 'Publishing…' : 'Publish post')}</button></div></Modal>; }
    if (modal === 'tipConfig') return <TipConfigModal onClose={closeModal} onNotify={notify} />;
  ...
  const studioWorkspace = <>{view === 'overview' && <Overview onOpen={setModal} onPlay={() => setPlaying(!playing)} playing={playing} onNavigate={navigate} />}{view === 'activity' && <ActivityView onOpen={setModal} onEditPost={openPostEditor} onDeletePost={handleDeletePost} />}{view === 'releases' && <Releases onOpen={setModal} onNotify={notify} playing={playing} onPlay={() => setPlaying(!playing)} focusProjectId={focusProjectId} onConsumeFocus={() => setFocusProjectId(null)} onManagePricing={(projectId) => goToProject('vault', projectId)} />}{view === 'vault' && <Vault onOpen={setModal} onNotify={notify} focusProjectId={focusProjectId} onConsumeFocus={() => setFocusProjectId(null)} onManageTracks={(projectId) => goToProject('releases', projectId)} />}{view === 'station' && <Station onNotify={notify} />}{view === 'schedule' && <ScheduleView onNotify={notify} />}{view === 'audience' && <AudienceView onNotify={notify} />}{view === 'broadcast' && <Broadcast onOpen={setModal} onNotify={notify} live={liveBroadcast.isLive} />}{view === 'analytics' && <Analytics onNotify={notify} />}{view === 'earnings' && <Earnings onOpen={setModal} onNotify={notify} />}{view === 'profile' && <ProfileView onNotify={notify} />}{view === 'tools' && <Tools onNotify={notify} onOpen={setModal} />}{view === 'security' && <Security onNotify={notify} />}{view === 'settings' && <SettingsView onNotify={notify} />}</>;
  const workspace = mode === 'stack' ? <StackView engine={playerEngine} onOpen={setModal} onNotify={notify} onVideoTrackSelected={() => switchMode('play')} /> : mode === 'play' ? <PlayerView engine={playerEngine} onOpen={setModal} onNotify={notify} onPlayButtonVisibilityChange={setPlayButtonVisible} /> : studioWorkspace;
  return <div className={cn('studio-app noise min-h-[100dvh]', mode !== 'studio' && 'listening-shell')}><audio ref={playerEngine.audioRef} hidden /><aside className={cn('sidebar-shell fixed z-50 inset-y-0 left-0 w-[248px] border-r border-white/[.07] flex flex-col md:translate-x-0', mode !== 'studio' && 'sidebar-hidden', mobileMenu ? 'mobile-drawer open' : 'mobile-drawer')}><div className="h-20 px-6 flex items-center gap-3 border-b border-white/[.07]"><a href="https://paperweighthq.com" target="_blank" rel="noopener noreferrer" className="flex items-center gap-3"><Logo size={32} /><div><p className="font-mono-ui text-[9px] uppercase tracking-[.18em] text-muted-foreground">Paperweight:</p><p className="font-display font-bold tracking-[-.03em]">STUDIO</p></div></a><IconButton label="Close mobile menu" onClick={() => setMobileMenu(false)} className="ml-auto md:hidden"><X size={16} /></IconButton></div><div className="px-4 py-5 flex items-center gap-3"><Avatar name={stationName} size="sm" /><div className="min-w-0"><p className="text-sm font-medium truncate">{stationName}</p></div><ChevronDown size={14} className="ml-auto text-muted-foreground" /></div><nav className="flex-1 px-3 overflow-y-auto sidebar-scroll">{navGroups.map((group) => <div key={group.label} className="mb-6"><p className="font-mono-ui text-[9px] uppercase tracking-[.2em] text-muted-foreground/60 px-3 mb-2">{group.label}</p>{group.items.map((item) => { const Icon = item.icon; return <button type="button" key={item.id} data-testid={`nav-${item.id}`} onClick={() => { switchMode('studio'); navigate(item.id); }} className={cn('nav-item w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm mb-1 text-left', view === item.id && 'active')}><Icon size={16} /><span className="flex-1">{item.label}</span>{item.badge && <span className="font-mono-ui text-[9px] min-w-5 h-5 rounded-full bg-muted text-foreground flex items-center justify-center">{item.badge}</span>}</button>; })}</div>)}<div className="mb-6"><p className="font-mono-ui text-[9px] uppercase tracking-[.2em] text-muted-foreground/60 px-3 mb-2">Account</p><button type="button" data-testid="nav-security" onClick={() => { switchMode('studio'); navigate('security'); }} className={cn('nav-item w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm mb-1 text-left', view === 'security' && 'active')}><ShieldCheck size={16} /> Security</button><button type="button" data-testid="nav-settings" onClick={() => { switchMode('studio'); navigate('settings'); }} className={cn('nav-item w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-left', view === 'settings' && 'active')}><SettingsIcon size={16} /> Settings</button></div></nav><div className="p-4 border-t border-white/[.07] space-y-2"><div className="panel-subtle rounded-xl p-3"><div className="flex items-center gap-2"><span className="h-1.5 w-1.5 rounded-full bg-primary" /><span className="font-mono-ui text-[9px] uppercase tracking-[.15em] text-primary">All systems clear</span></div><p className="text-[11px] text-muted-foreground mt-2">Last synced just now</p></div><button type="button" data-testid="button-sign-out" onClick={logout} className="nav-item w-full flex items-center gap-3 px-3 py-2 rounded-lg text-xs text-left text-muted-foreground"><LogOut size={14} /> Sign out</button></div></aside><div className={cn('fixed inset-0 z-40 bg-black/60 backdrop-blur-md md:hidden mobile-overlay', mobileMenu && 'open')} onClick={() => setMobileMenu(false)} /><main className={cn('min-h-[100dvh]', mode === 'studio' ? 'md:pl-[248px]' : 'mode-main')}><header className="relative h-20 px-4 sm:px-8 lg:px-10 border-b border-white/[.07] flex items-center gap-4 sticky top-0 z-30 glass-header"><button type="button" aria-label="Open navigation menu" data-testid="button-open-mobile-menu" onClick={() => setMobileMenu(true)} className={cn('md:hidden ghost-button h-9 w-9 rounded-lg', mode !== 'studio' && 'hidden')}><Menu size={18} /></button><div className={cn('absolute left-1/2 -translate-x-1/2', mode === 'studio' && 'md:left-[calc(50%-124px)]')}><ModeSwitcher mode={mode} onChange={switchMode} /></div><div className="hidden lg:flex items-center gap-2 text-xs text-muted-foreground"><span>{mode === 'stack' ? 'Stack' : mode === 'play' ? 'Play' : 'Studio'}</span><ChevronRight size={13} /><span className="text-foreground">{mode === 'studio' ? currentTitle : mode === 'stack' ? 'Library' : 'Player'}</span></div><div className="ml-auto flex items-center gap-2"><button type="button" data-testid="button-preview-profile" onClick={() => notify('Public profile preview opened.')} className="hidden sm:flex ghost-button rounded-lg px-3 py-2 text-xs items-center gap-2"><Eye size={14} /> Preview profile</button><IconButton label="Open notifications" onClick={() => notify('No new notifications beyond your 4 activity signals.')}><Bell size={16} /></IconButton><button type="button" data-testid="button-header-avatar" onClick={() => { switchMode('studio'); navigate('profile'); }} className="rounded-full"><Avatar name={stationName} size="sm" /></button></div></header><div className={cn('p-5 sm:p-8 lg:p-10 max-w-[1480px] mx-auto', mode !== 'studio' && 'mode-content')}><AnimatePresence mode="wait" initial={false} custom={modeTransitionDirection}><motion.div key={mode} custom={modeTransitionDirection} variants={modeViewVariants} initial="enter" animate="center" exit="exit" transition={reduceMotion ? { duration: 0.12 } : { type: 'spring', bounce: 0, duration: 0.32 }}>{workspace}</motion.div></AnimatePresence></div></main><StickyTransport engine={playerEngine} visible={mode !== 'play' || !playButtonVisible} offsetForSidebar={mode === 'studio'} onNotify={notify} />{<AnimatePresence>{modalContent()}</AnimatePresence>}{toast && <div data-testid="status-toast" className="fixed bottom-5 left-1/2 -translate-x-1/2 z-[90] panel rounded-xl px-4 py-3 text-sm flex items-center gap-2 shadow-2xl animate-enter"><Check size={15} className="text-primary" /> {toast}</div>}</div>;
}
