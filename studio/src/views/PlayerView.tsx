...
export function PlayerView({ engine, onOpen, onNotify }: { engine: PlayerEngine; onOpen: (modal: ModalKey) => void; onNotify: (message: string) => void }) {
  const { playing, reconnecting, toggle, stationName, nowPlaying, listenerCount, liveActive, stationQueue, recentlyPlayed, track, isPreview, odProgress, odElapsed, quota, goLive } = engine;
    ...
    <div className="animate-enter player-workspace">
      ...
      <div className="player-stage panel rounded-[2rem] overflow-hidden">
        ...
        <div className="player-copy">
          <span className="font-mono-ui text-[10px] uppercase tracking-[.22em] text-primary flex items-center gap-2">
            {!track && liveActive && <span className="h-1.5 w-1.5 rounded-full bg-destructive animate-pulse" />}
            {track ? (isPreview ? 'Preview' : 'On demand') : reconnecting ? 'Reconnecting…' : liveActive ? 'Live now' : playing ? 'Playing now' : 'Station paused'}
          </span>
      ...
      <div className="player-lower-grid">
        <section className="panel rounded-2xl p-5 sm:p-6"><div className="flex items-center justify-between mb-4"><div><p className="font-mono-ui text-[10px] uppercase tracking-[.2em] text-muted-foreground">Rotation</p><h2 className="font-display text-xl font-semibold mt-1">Up next</h2></div></div>
          {stationQueue.length ? stationQueue.slice(0, 4).map((queued) => <div key={queued.id} className="flex items-center gap-3 py-2.5 border-b border-white/[.07] last:border-0"><span className="flex-1 min-w-0 text-sm truncate">{queued.title}</span><span className="font-mono-ui text-[11px] text-muted-foreground">{formatDuration(queued.duration)}</span></div>)
          : <p className="text-sm text-muted-foreground py-4">Nothing queued — the station is shuffling the full catalog.</p>}
        </section>
        <section className="panel rounded-2xl p-5 sm:p-6">
          {recentlyPlayed.length ? <><div className="flex items-center gap-3 mb-4"><span className="h-9 w-9 rounded-xl bg-primary/10 text-primary flex items-center justify-center"><Radio size={16} /></span><div><p className="font-mono-ui text-[10px] uppercase tracking-[.2em] text-primary">Just played</p><h2 className="font-display text-xl font-semibold mt-1">Recently on air.</h2></div></div>{recentlyPlayed.slice(0, 3).map((played) => <div key={`${played.id}-${played.playedAt}`} className="flex items-center gap-3 py-2 text-sm"><span className="flex-1 min-w-0 truncate">{played.title}</span>{played.artist && <span className="text-xs text-muted-foreground truncate">{played.artist}</span>}</div>)}</> : <><div className="flex items-center gap-3"><span className="h-9 w-9 rounded-xl bg-primary/10 text-primary flex items-center justify-center"><Radio size={16} /></span><div><p className="font-mono-ui text-[10px] uppercase tracking-[.2em] text-primary">Private signal</p><h2 className="font-display text-xl font-semibold mt-1">Keep the room close.</h2></div></div><p className="text-sm text-muted-foreground mt-5 leading-relaxed">Share a listening link with the people who should hear it first.</p><button type="button" data-testid="button-player-private-share" onClick={() => onOpen('share')} className="ghost-button rounded-xl px-3 py-2.5 text-xs mt-5 flex items-center gap-2"><Link2 size={14} /> Create private link</button></>}
        </section>
      </div>
    </div>
  );
