  ...
  return (
    <div className="animate-enter">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        ...
        <Metric label="Listening hours" value="—" change="Wired in a later pass" icon={Clock3} accent="blue" />
        <Metric label="This month" value="—" change="Wired in a later pass" icon={Wallet} />
      </div>
      <div className="grid lg:grid-cols-[1.12fr_.88fr] gap-6">
        ...
        <section className="panel rounded-2xl p-5 sm:p-6"><div className="flex items-center justify-between mb-5"><div><p className="font-mono-ui text-[10px] uppercase tracking-[.2em] text-muted-foreground">Pulse check</p><h2 className="font-display text-xl font-semibold mt-1">Audience, this week</h2></div><button type="button" data-testid="button-view-analytics" onClick={() => onNavigate('analytics')} className="ghost-button rounded-lg px-2.5 py-1.5 text-xs">Details</button></div><div className="h-28 flex items-end gap-2 mb-3">{[38,52,44,68,59,82,74,91,77,100,84,95,89,100].map((height, i) => <div key={i} className="flex-1 rounded-t-sm bg-primary/70 hover:bg-primary transition-colors" style={{ height: `${height}%` }} />)}</div><div className="flex justify-between text-[10px] font-mono-ui text-muted-foreground"><span>Mar 08</span><span>Mar 14</span><span>Mar 21</span></div><p className="text-[10px] text-muted-foreground mt-3">Placeholder chart — real analytics wiring is a later pass.</p></section>
      </div>
