/**
 * api.js — Centralized fetch layer, ported verbatim from client/js/api.js
 * (creator.html's fetch layer) so studio/ calls the exact same endpoints.
 * Keep in sync by hand until creator.html is retired — see CLAUDE.md.
 *
...
export const auth = {
  ...
  /**
   * POST /api/auth/dashboard/logout — clears the dashboard session cookie.
   * Backend route already existed (src/api/auth.js) but had no caller in
   * creator.html — added here for the studio/ dashboard's sign-out control.
   * @returns {Promise<void>}
   */
  async dashboardLogout() {
    await _fetch('/api/auth/dashboard/logout', { method: 'POST' });
  },
};
