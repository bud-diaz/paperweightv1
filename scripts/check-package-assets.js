...
const { assertLinuxElfBufferArch, parseOptions } = require('./lib/binary-target');

const ROOT = path.resolve(__dirname, '..');
let ok = true;
const OPTIONS = parseOptions(process.argv.slice(2));
const REQUIRE_BINARY_BUNDLES = OPTIONS['require-binary-bundles'] === true || process.env.PAPERWEIGHT_REQUIRE_BINARY_BUNDLES === '1';
const EXPECTED_PLATFORM = OPTIONS.platform || process.env.PAPERWEIGHT_EXPECT_PLATFORM || null;
const EXPECTED_ARCH = OPTIONS.arch || process.env.PAPERWEIGHT_EXPECT_ARCH || null;
const EXPECTED_ABI = String(OPTIONS.abi || process.env.PAPERWEIGHT_EXPECT_ABI || '115');
...
if (fs.existsSync(nativeBundlePath)) {
  try {
    ...
    const nativeData = Buffer.isBuffer(nativeBundle) ? nativeBundle : nativeBundle.data;
    if (!hasNativeData) {
      fail('native bundle malformed: missing better_sqlite3.node data');
    } else if (nativeAbi !== EXPECTED_ABI && REQUIRE_BINARY_BUNDLES) {
      ...
      pass(`native bundle ABI: ${nativeAbi || 'unknown'}`);
    }
    if (hasNativeData && EXPECTED_PLATFORM && nativeBundle.platform !== EXPECTED_PLATFORM) {
      fail(`native bundle platform mismatch: ${nativeBundle.platform || 'unknown'} !== ${EXPECTED_PLATFORM}`);
    }
    if (hasNativeData && EXPECTED_ARCH && nativeBundle.arch !== EXPECTED_ARCH) {
      fail(`native bundle architecture mismatch: ${nativeBundle.arch || 'unknown'} !== ${EXPECTED_ARCH}`);
    }
    if (hasNativeData && nativeBundle.sha256 && crypto.createHash('sha256').update(nativeData).digest('hex') !== nativeBundle.sha256) {
      fail('native bundle hash mismatch');
    }
    if (hasNativeData && EXPECTED_PLATFORM === 'linux' && EXPECTED_ARCH) {
      try {
        assertLinuxElfBufferArch(nativeData, EXPECTED_ARCH, 'native bundle');
        pass(`native bundle ELF architecture: ${EXPECTED_ARCH}`);
      } catch (err) {
        fail(err.message);
      }
    }
...
if (fs.existsSync(ffmpegBundlePath)) {
  try {
    ...
    if (isFfmpegBundleEntry(ffmpegEntry) && isFfmpegBundleEntry(ffprobeEntry)) {
      ...
      if (EXPECTED_PLATFORM && ffmpegBundle.platform !== EXPECTED_PLATFORM) {
        fail(`ffmpeg bundle platform mismatch: ${ffmpegBundle.platform || 'unknown'} !== ${EXPECTED_PLATFORM}`);
      }
      if (EXPECTED_ARCH && ffmpegBundle.arch !== EXPECTED_ARCH) {
        fail(`ffmpeg bundle architecture mismatch: ${ffmpegBundle.arch || 'unknown'} !== ${EXPECTED_ARCH}`);
      }
      for (const [name, entry] of [['ffmpeg', ffmpegEntry], ['ffprobe', ffprobeEntry]]) {
        try {
          const encoded = Buffer.isBuffer(entry.data) ? entry.data : Buffer.from(Array.isArray(entry.data) ? entry.data.join('') : entry.data, 'base64');
          const raw = entry.compressed === true || entry.compression === 'gzip' ? zlib.gunzipSync(encoded) : encoded;
          if (entry.sha256 && crypto.createHash('sha256').update(raw).digest('hex') !== entry.sha256) {
            throw new Error(`${name} bundle hash mismatch`);
          }
          if (EXPECTED_PLATFORM === 'linux' && EXPECTED_ARCH) {
            assertLinuxElfBufferArch(raw, EXPECTED_ARCH, `${name} bundle`);
          }
          pass(`${name} bundle validated${EXPECTED_ARCH ? ` for ${EXPECTED_ARCH}` : ''}`);
        } catch (err) {
          fail(err.message);
        }
      }
